...
function recoverForm() {
    ...
    const formBindStageEmail = form.bind(stageEmailForm, event, (data) => validate(data, false, false, true, false), (data) => recoverStageEmail(data), () => window.location.href = "/views/recovery.html?code");
    const formBindStageCode = form.bind(stageCodeForm, event, () => { }, // we don't need to validate the verification code
    (data) => recoverStageCode(data), () => window.location.href = "/views/recovery.html?password");
    const formBindStagePassword = form.bind(stagePasswordForm, event, (data) => validate(data, false, true, false, true), (data) => recoverStagePassword(data), () => window.location.href = "/views/login.html?sign-in");
