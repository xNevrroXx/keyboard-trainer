...
// Functions
  function initTraining() {
    let indexTargetChar: number = 0,
      pastTime: number = 0,
      failedTargetChar: string | null = null;

    const statisticSpeedValueElem = statisticElem.querySelector(".statistic__speed > .value"),
      statisticAccuracyElem = statisticElem.querySelector(".statistic__accuracy .value");

    setInterval(() => {
      pastTime++;
      const accuracy = buffer.length !== 0 ? Math.floor(100 - countErrors / (buffer.length / 100)) : 100,
        speed = Math.floor(buffer.length / pastTime);
  ...
  function toggleKeyClass(code: string, neededClass: string) {
